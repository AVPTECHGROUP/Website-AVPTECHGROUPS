...
const Navbar = () => {
    ...
    const navLinks = ['Home', 'Features', 'Highlights', 'Clients', 'Blog'];

    return (
        <div className="relative bg-theme-nav text-theme-text transition-colors duration-300 w-full">
            {/* Scoped Premium Shimmer Animation Styles */}
            <style>{`
            ...
            {/* ── Top Announcement Bar (Visible on md and up) ── */}
            <div className="hidden md:flex px-6 lg:px-12 xl:px-16 py-2 items-center justify-between border-b border-theme-border/40">
                ...
                <div className='flex gap-4 lg:gap-6'>
                    <a
                        ...
                        className='flex items-center gap-1.5 text-theme-subtext hover:text-[#00C9B1] font-body text-xs lg:text-sm transition-colors duration-200'
                    >
            ...
            <div className='flex items-center justify-between px-4 sm:px-8 lg:px-12 xl:px-16 py-3 relative max-w-7xl mx-auto w-full'>
                ...
                <div className="flex-shrink-0 cursor-pointer" onClick={() => navigate('/')}>
                    <img
                        className="h-12 sm:h-14 lg:h-16 xl:h-20 w-auto object-contain drop-shadow-md hover:drop-shadow-xl transition duration-300 ease-in-out hover:scale-105"
                        src={ss_logo_2}
                        alt="SchoolSpine Logo"
                    />
                </div>

                {/* Desktop Menu Links & Actions (Visible on lg and up) */}
                <div className='hidden lg:flex items-center justify-between flex-1 ml-8 xl:ml-12 z-50'>
                    ...
                    {/* Desktop Premium Buttons Container */}
                    <div className="flex items-center gap-4 xl:gap-5">
                        {/* Theme Toggle Button */}
                        <button
                            onClick={toggleTheme}
                            className="p-2.5 rounded-xl border border-theme-border bg-theme-card text-theme-text hover:bg-theme-border/20 transition-all duration-300 cursor-pointer flex items-center justify-center"
                            aria-label="Toggle theme"
                        >
                            {theme === 'dark' ? <Sun size={18} className="text-[#F5A623]" /> : <Moon size={18} className="text-[#00C9B1]" />}
                        </button>

                        {/* Premium Login Button - UPDATED with matching Gradient & Shimmer */}
                        <button
                        ...
                        {/* Premium CTA Button: Get Free Demo */}
                        <button
                            className="group relative overflow-hidden flex items-center gap-2 px-6 xl:px-7 py-2.5 rounded-xl font-bold tracking-wide text-slate-950 cursor-pointer bg-gradient-to-r from-[#00C9B1] via-[#00C9B1] to-[#F5A623] shadow-[0_4px_20px_rgba(0,201,177,0.25)] hover:shadow-[0_6px_24px_rgba(245,166,35,0.35)] hover:-translate-y-0.5 transition-all duration-300 active:scale-[0.98]"
                        >
                            <div className="shimmer-effect" />
                            <span className="text-sm xl:text-base relative z-10">Get Free Demo</span>
                            <div className="relative z-10 bg-slate-950/10 rounded-lg p-1 flex items-center justify-center transition-transform duration-300 group-hover:translate-x-1 group-hover:bg-slate-950/20">
                                <ArrowRight size={14} className="text-slate-950 stroke-[2.5]" />
                            </div>
                        </button>
                    </div>
                </div>
                ...
                <div className='flex lg:hidden gap-3 items-center'>
                    ...
                    <div className='flex md:hidden gap-3 items-center text-theme-text text-xs border-r border-theme-border pr-2 mr-1'>
                        <a href="tel:+919511117450" className='flex items-center gap-1 text-nowrap hover:text-[#00C9B1]'>
                            <Phone size={12} className="text-theme-text" /> Call
                        </a>
                        <a href="mailto:info@computesofttech.com" className='flex items-center gap-1 text-nowrap hover:text-[#00C9B1]'>
                            <Mail size={12} className="text-theme-text" /> Email
                        </a>
                    </div>

                    <button
                        className='p-1 text-[#00C9B1] cursor-pointer hover:scale-105 transition-transform'
            ...
            {menuOpen && (
                <div className='lg:hidden fixed inset-0 z-50 bg-theme-bg text-theme-text flex flex-col transition-colors duration-300 animate-in fade-in zoom-in-95 duration-200'>
                    ...
                    <div className='px-6 mt-auto pb-8 flex flex-col gap-3 border-t border-theme-border/40 pt-4'>
                        {/* Mobile Login Button - Matches Color Profile */}
                        <button
                            onClick={() => { window.open("/login", "_blank"); setMenuOpen(false); }}
                            className='w-full py-3 text-sm relative overflow-hidden flex items-center justify-center gap-2 text-slate-950 bg-gradient-to-r from-[#00C9B1] to-[#F5A623] rounded-xl font-bold cursor-pointer transition-transform duration-200 shadow-md'
                        >
                            <div className="shimmer-effect" />
