...
const ONES_WORDS = ['', 'One', 'Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten',
    'Eleven', 'Twelve', 'Thirteen', 'Fourteen', 'Fifteen', 'Sixteen', 'Seventeen', 'Eighteen', 'Nineteen']
const TENS_WORDS = ['', '', 'Twenty', 'Thirty', 'Forty', 'Fifty', 'Sixty', 'Seventy', 'Eighty', 'Ninety']

function twoDigitWords(n) {
    if (n < 20) return ONES_WORDS[n]
    return TENS_WORDS[Math.floor(n / 10)] + (n % 10 ? '-' + ONES_WORDS[n % 10] : '')
}
...
function threeDigitWords(n) {
    ...
    const rest = n % 100
    let str = ''
    if (hundred) str += ONES_WORDS[hundred] + ' Hundred' + (rest ? ' ' : '')
...
/**
 * Maps a salary slip record (teacher/employee details + period + attendance +
 * itemized earnings/deductions + bank details) + school details into the data
 * object expected by the SALARY_SLIP template merge engine.
 *
 * `record.earnings` / `record.deductions` are arrays of { label, amount },
 * mirroring the itemized rows shown on the printed slip — gross salary, total
 * deductions, net pay, and the amount-in-words line are all derived here so
 * screens don't have to duplicate that math.
 */
export function buildSalarySlipMergeData(record = {}, school = {}) {
    const schoolName = school?.name || school?.schoolName || 'School Name'
    ...
    return {
        ...
        payrollStatus: record.payrollStatus || 'PROCESSED',
        ...
        earnings: earnings.map((e) => ({ label: e.label, amount: formatCurrency(e.amount) })),
        deductions: deductions.map((d) => ({ label: d.label, amount: formatCurrency(d.amount) })),
    }
}
