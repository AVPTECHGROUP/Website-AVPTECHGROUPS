...
function UploadZone({ file, onFileChange, disabled, existingUrl, existingType }) {
  ...
  return (
    <div className="space-y-3">
      {existingUrl && !file && (
        <LazyAttachmentViewer url={existingUrl} type={existingType} label="Current Attachment" />
      )}
      <div
      ...
      >
        <input ref={ref} type="file" className="hidden" accept=".pdf,.png,.jpg,.jpeg"
...
function Tab({ active, onClick, icon: Icon, label, dot }) {
  return (
    ...
    >
      <Icon size={13} />
      {label}
      {dot && <span className="w-1.5 h-1.5 rounded-full bg-blue-500 absolute top-2 right-1.5" />}
    </button>
  );
}
...
function resolveTeacherName(t) {
  ...
  const fromFields = (
    t.name ?? t.fullName ?? t.profile?.fullName ??
...
export default function AssignModal({
  ...
  useEffect(() => {
    if (isEdit && hw?.subjectId && subjects.length > 0) {
      ...
      if (match) setForm((f) => ({ ...f, subjectId: String(hw.subjectId) }));
    }
  }, [subjects, isEdit, hw?.subjectId]);

  // ── When academic years load in edit mode, ensure academicYearId is set ───
  useEffect(() => {
    if (isEdit && hw?.academicYearId && academicYears.length > 0) {
      setForm((f) => ({ ...f, academicYearId: String(hw.academicYearId) }));
    }
  }, [academicYears, isEdit, hw?.academicYearId]);
      ...
      >
          ...
          {activeTab === "details" && (
            <div className="space-y-4">
              ...
              {isTeacher ? (
                ...
                <Field label="On behalf of teacher" required>
                  ...
                  {!teachersLoading && teachers.length === 0 && (
                    <p className="text-[10px] text-red-400 mt-1">No teachers loaded — check console for [getTeacherLookup] logs.</p>
                  )}
                </Field>
              )}
              ...
              <Field label="Title" required>
                <input type="text" value={form.title} disabled={submitting}
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Assigned Date" required>
                  <input type="date" value={form.assignedDate} disabled={submitting}
                    onChange={(e) => set("assignedDate", e.target.value)}
                    className="w-full border-[1.5px] border-gray-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:opacity-60"
                  />
                </Field>
                <Field label="Due Date" required>
                  <input type="date" value={form.dueDate} disabled={submitting}
          ...
          {activeTab === "content" && (
            <div className="space-y-4">
              <Field label="Description">
                <textarea value={form.description} disabled={submitting}
                  onChange={(e) => { if (e.target.value.length <= 1000) set("description", e.target.value); }}
                  placeholder="Instructions for students…" rows={6}
                  className="w-full border-[1.5px] border-gray-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 resize-none overflow-y-auto disabled:opacity-60"
              ...
              {showUpload && (
                <UploadZone file={attachFile} onFileChange={setAttachFile} disabled={submitting}
                  existingUrl={existingAttachUrl} existingType={existingAttachType} />
              )}

              {showLink && (
                <Field label="Resource Link">
                  <div className="space-y-2">
                    <input type="url" value={form.linkUrl} disabled={submitting}
                      onChange={(e) => set("linkUrl", e.target.value)}
                      placeholder="https://…"
                      className="w-full border-[1.5px] border-gray-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:opacity-60"
                    />
                    {form.linkUrl && <LazyAttachmentViewer url={form.linkUrl} type="link" label="Link Preview" />}
                  </div>
                </Field>
              )}
            </div>
        ...
        <div className="flex items-center justify-between gap-2.5 px-5 py-3.5 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {!isOnDetails && (
              <>
                <button type="button" onClick={() => setActiveTab("details")} disabled={submitting}
