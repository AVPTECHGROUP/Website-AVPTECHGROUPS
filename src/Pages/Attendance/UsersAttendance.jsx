...
const ApproveModal = ({ user, onClose, onConfirm }) => {
  ...
  const handleConfirm = async () => {
    if (!remarks.trim()) { toast.error('Please enter approval remarks'); return; }
    setLoading(true);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-gray-900">Approve Attendance</h3>
          </div>
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-gray-900 text-sm">{user.userName || 'N/A'}</p>
                  <p className="text-xs text-gray-500 mt-0.5">Manual attendance request</p>
                </div>
              </div>
              <span className="px-2.5 py-1 bg-yellow-50 text-yellow-700 ring-1 ring-yellow-200 text-xs font-semibold rounded-lg shrink-0">Pending Review</span>
            </div>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3">
              <span className="flex items-center gap-1 text-xs text-gray-500">
                <Calendar className="w-3.5 h-3.5" /> Date: {user.attendanceDate}
              </span>
              {user.checkInTime && (
                <span className="flex items-center gap-1 text-xs text-gray-500">
                  <Clock className="w-3.5 h-3.5" /> Submitted: {user.checkInTime}
                </span>
              )}
              <span className="flex items-center gap-1 text-xs text-gray-500">
                <AlertTriangle className="w-3.5 h-3.5" /> Manual entry
              </span>
            </div>
            {user.remarks && (
          ...
          {/* Override Status */}
          <div>
            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1.5">Override Status</label>
            <div className="relative">
              <select value={overrideStatus} onChange={(e) => setOverrideStatus(e.target.value)}
                className="w-full appearance-none px-4 py-2.5 pr-10 border border-gray-300 rounded-xl bg-white text-sm text-gray-700 font-medium focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all cursor-pointer">
                <option value="PRESENT">PRESENT — On-time arrival</option>
                <option value="LATE">LATE — After grace period</option>
                <option value="HALF_DAY">HALF_DAY — Partial attendance</option>
              </select>
          ...
          {/* Remarks */}
          <div>
            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1.5">Approval Remarks</label>
            <textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} rows={3}
              placeholder="e.g. Verified with CCTV footage. Identity confirmed."
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
          <button onClick={onClose} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-all">Cancel</button>
          <button onClick={handleConfirm} disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-green-600 hover:bg-green-700 disabled:opacity-60 rounded-xl text-sm font-semibold text-white transition-all">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-gray-900">Reject Attendance</h3>
          </div>
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          {/* Rejection Reason */}
          <div>
            <label className="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-1.5">Rejection Reason <span className="text-red-500">*</span></label>
            <textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} rows={3}
              placeholder="e.g. No valid justification provided. Please contact the HR department."
            ...
            <p className="text-xs text-gray-400 mt-1">This reason will be recorded in the audit log and communicated to the employee.</p>
          </div>
        </div>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
          <button onClick={onClose} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-all">Cancel</button>
          <button onClick={handleConfirm} disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-700 disabled:opacity-60 rounded-xl text-sm font-semibold text-white transition-all">
...
const UsersAttendance = () => {
  ...
  const handleApproveConfirm = async ({ attendanceId, approved, remarks, overrideStatus }) => {
    try {
      ...
      toast.success(`${approveModal?.userName || 'User'} — ${res?.message || 'Approved successfully'}`);
  ...
  const handleRejectConfirm = async ({ attendanceId, approved, remarks }) => {
    try {
      ...
      toast.success(`${rejectModal?.userName || 'User'} — Rejected successfully`);
      setRejectModal(null);
    } catch (err) {
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        <div className="flex-1 bg-gradient-to-b from-sky-50 to-sky-100 overflow-auto p-3 sm:p-4 md:p-6 lg:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-5">
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Pending Approvals</h2>
              <p className="text-gray-500 mt-1 text-xs sm:text-sm font-medium">Manage and track daily attendance pending records for all staff.</p>
            </div>
            ...
            {pendingUsers.length > 0 && (
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
                <div className="text-sm text-gray-600 font-medium">
                  Showing {startIndex + 1}–{Math.min(endIndex, pendingUsers.length)} of {pendingUsers.length} records
                </div>
                <div className="flex items-center gap-1.5">
                  <button onClick={() => setCurrentPage(p => Math.max(p - 1, 1))} disabled={currentPage === 1}
                    className="w-8 h-8 rounded-lg border border-gray-300 flex items-center justify-center hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                  ...
                  {[...Array(totalPages)].map((_, i) => {
                    ...
                    if (p === 1 || p === totalPages || (p >= currentPage - 1 && p <= currentPage + 1))
                      return <button key={p} onClick={() => setCurrentPage(p)}
                        className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-medium transition-all ${currentPage === p ? 'bg-blue-600 text-white shadow-sm' : 'border border-gray-300 text-gray-600 hover:bg-gray-50'}`}>{p}</button>;
              ...
              <div key={user.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-5">
                <div className="flex items-start justify-between mb-3 pb-3 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-900 text-sm">{user.userName || 'N/A'}</h3>
                      <p className="text-xs text-gray-400 mt-0.5">ID: {user.userId}</p>
                    </div>
                  </div>
