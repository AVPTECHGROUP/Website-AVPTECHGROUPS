...
import { allAttendanceDetails } from '../../Api/AttendanceApi';
...
const Attendance = () => {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const { getAllUserRoles } = await import('../../Api/userManagementAPI');
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const { attendanceStatistics } = await import('../../Api/AttendanceApi');
  ...
  // Called after successful manual submission — refresh list
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(attendanceList, filename);
  };
        ...
        <div className="flex-1 bg-linear-to-b from-sky-50 to-sky-100 overflow-auto p-3 sm:p-4 md:p-6 lg:p-8">
          ...
          <div className="mb-5">
            <div className="grid grid-cols-2 gap-2 sm:hidden">
              {TABS.map((tab) => {
                const Icon = tab.icon; const active = tab.id === 'attendanceOverview';
                return (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div className="flex flex-wrap gap-2">
              <button onClick={() => navigate('/attendance/markUserAttendance')} className="flex items-center gap-2 px-4 py-2.5 bg-blue-700 hover:bg-blue-800 text-white text-sm font-semibold rounded-xl shadow-sm transition-all">
                <ScanFace className="w-4 h-4" />
                <span className="hidden sm:inline">Individual Face Scan</span>
                <span className="sm:hidden">Face Scan</span>
              </button>
              <button onClick={() => setManualMarkOpen(true)} className="flex items-center gap-2 px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold rounded-xl shadow-sm transition-all">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 mb-4">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                <select value={selectedRole} onChange={(e) => { setSelectedRole(e.target.value); setPage(0); }}
                  ...
                  {roles.map((r) => (
                    <option key={r.id || r.name || r} value={r.name || r}>{r.name || r}</option>
                  ))}
                </select>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 sm:px-6 py-4 border-t border-gray-200 bg-gray-50">
              <div className="flex items-center gap-3 text-sm">
                <span className="text-gray-600 font-medium text-xs sm:text-sm">Showing {startRow}–{endRow} of {pagination?.totalElements || 0}</span>
              ...
              <div className="flex items-center gap-1.5">
                <button disabled={!pagination?.hasPrevious} onClick={() => setPage(p => p - 1)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${pagination?.hasPrevious ? 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>← Prev</button>
                ...
                <button disabled={!pagination?.hasNext} onClick={() => setPage(p => p + 1)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${pagination?.hasNext ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>Next →</button>
              </div>
            </div>
          </div>
