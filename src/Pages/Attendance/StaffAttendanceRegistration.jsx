...
import {
    ...
    RefreshCw, ChevronLeft, ChevronRight, Trash2, X, FileText, Zap,
    Shield, Camera, AlertCircle, Upload,
} from "lucide-react";
import CardComponent from "../../Components/CommonComp/CardComponent";
import CardLoader from "../../Components/CommonComp/CardLoader";
import {
    getEnrollmentStats, getStaffEnrollment, enrollUserFaces, removeEnrollment,
} from "../../Api/Attendance/AttendanceApi";
import { STAFF_AVATAR_COLORS as AVATAR_COLORS, ROLE_COLORS, UI_STRINGS } from "../../Constants/StringConstants/AttendanceConstants";
    ...
    if (status === "ENROLLED")
        return (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-semibold border border-emerald-200">
                <CheckCircle className="w-3.5 h-3.5" /> {UI_STRINGS.STAFF_ENROLL.ENROLLED}
            </span>
        );
    if (status === "PARTIAL")
        return (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-semibold border border-amber-200">
                <AlertTriangle className="w-3.5 h-3.5" /> {UI_STRINGS.STAFF_ENROLL.PARTIAL} {photos}/5
            </span>
        );
    return (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-50 text-red-600 text-xs font-semibold border border-red-200">
            <AlertTriangle className="w-3.5 h-3.5" /> {UI_STRINGS.STAFF_ENROLL.NOT_ENROLLED}
        </span>
    );
}
    ...
    if (status === "ENROLLED")
        return (
            <button onClick={onReEnroll} className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold transition-colors border border-gray-200">
                <RefreshCw className="w-3.5 h-3.5" /> {UI_STRINGS.STAFF_ENROLL.BTN_REENROLL}
            </button>
        );
    if (status === "PARTIAL")
        return (
            <button onClick={onEnroll} className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold transition-colors">
                <Camera className="w-3.5 h-3.5" /> {UI_STRINGS.STAFF_ENROLL.BTN_COMPLETE}
            </button>
        );
    return (
        <button onClick={onEnroll} className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition-colors">
            <Zap className="w-3.5 h-3.5" /> {UI_STRINGS.STAFF_ENROLL.BTN_ENROLL}
        </button>
    );
}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full border border-gray-200">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h3 className="text-base font-bold text-gray-900">{UI_STRINGS.STAFF_ENROLL.MODAL_HEADER}</h3>
                        <p className="text-xs text-gray-500 mt-0.5">{UI_STRINGS.STAFF_ENROLL.MODAL_SUB}</p>
                    </div>
                </div>
                ...
                <p className="text-sm text-gray-600 mb-5 leading-relaxed">
                    {UI_STRINGS.STAFF_ENROLL.MODAL_WARNING}
                </p>
                <div className="flex gap-3">
                    ...
                    <button onClick={onConfirm} disabled={loading} className="cursor-pointer flex-1 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-semibold transition-colors flex items-center justify-center gap-2">
                        {loading ? <><RefreshCw className="w-4 h-4 animate-spin" /> {UI_STRINGS.STAFF_ENROLL.REMOVING}</> : UI_STRINGS.STAFF_ENROLL.BTN_YES_REENROLL}
                    </button>
                </div>
            </div>
        </div>
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    <div>
                        <h3 className="text-sm font-bold text-gray-900">
                            {UI_STRINGS.STAFF_ENROLL.PHOTO_SLOT} {slotIndex + 1} {UI_STRINGS.STAFF_ENROLL.ADD_FACE}
                        </h3>
            ...
            <div className="relative">
                ...
                {file && (
                    <div className="absolute bottom-1 left-1 right-1 flex justify-center">
                        <span className="text-[10px] bg-emerald-600 text-white px-1.5 py-0.5 rounded font-semibold">{UI_STRINGS.STAFF_ENROLL.GOOD}</span>
                    </div>
                )}
            </div>
        </>
...
export default function StaffAttendanceRegistration() {
    ...
    useEffect(() => {
        ...
        setSelectedStaff((prev) => {
            if (prev.enrollmentStatus === live.enrollmentStatus && prev.photosCount === (live.photosCount || 0)) return prev;
            return {
                ...prev,
    ...
    const scrollToUpload = () => setTimeout(() => uploadPanelRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
    ...
    const paginatedStaff = tableFilteredData.slice(tablePage * tablePageSize, (tablePage + 1) * tablePageSize);
    const tableTotalPages = Math.max(1, Math.ceil(tableFilteredData.length / tablePageSize));

    return (
        <div className="min-h-screen bg-[#EBF0F5] font-sans">
            {reEnrollTarget && (
                <ReEnrollModal staff={reEnrollTarget} onConfirm={confirmReEnroll} onCancel={() => setReEnrollTarget(null)} loading={reEnrollLoading} />
            )}
            ...
            <div className="px-4 sm:px-6 lg:px-8 py-4">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <h1 className="text-xl sm:text-3xl font-bold text-gray-900">{UI_STRINGS.STAFF_ENROLL.HEADER}</h1>
                        <p className="text-sm text-gray-500 mt-0.5">{UI_STRINGS.STAFF_ENROLL.SUBTITLE}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {statsLoading ? Array(4).fill(0).map((_, i) => <CardLoader key={i} />) : (
                        <>
                            <CardComponent IconName={Shield} keyName={UI_STRINGS.STAFF_ENROLL.STAT_REG} val={`${stats?.totalStaff ?? 0}`} iconTxColor="text-blue-600" iconBgColor="bg-blue-100" />
                            <CardComponent IconName={UserCheck} keyName={UI_STRINGS.STAFF_ENROLL.STAT_ENROLLED} val={`${stats?.staffEnrolled ?? 0} / ${stats?.totalStaff ?? 0}`} iconTxColor="text-emerald-600" iconBgColor="bg-emerald-100" />
                            <CardComponent IconName={UserX} keyName={UI_STRINGS.STAFF_ENROLL.STAT_NOT_ENROLLED} val={`${(stats?.totalStaff ?? 0) - (stats?.staffEnrolled ?? 0)}`} iconTxColor="text-red-500" iconBgColor="bg-red-100" />
                            <CardComponent IconName={Users} keyName={UI_STRINGS.STAFF_ENROLL.STAT_COMPLETED} val={`${stats?.enrollmentPercentage ?? 0}%`} iconTxColor="text-purple-600" iconBgColor="bg-purple-100" />
                        </>
                    )}
                </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 h-full">
                            <h2 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                                <Users className="w-4 h-4 text-blue-500" /> {UI_STRINGS.STAFF_ENROLL.SELECT_STAFF}
                                {!allStaffLoading && <span className="ml-auto text-xs font-normal text-gray-400">{allStaff.length} {UI_STRINGS.STAFF_ENROLL.STAFF_TXT}</span>}
                            </h2>
                            <div className="relative mb-3">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                                <input value={userSearch} onChange={(e) => setUserSearch(e.target.value)}
                                    placeholder={UI_STRINGS.STAFF_ENROLL.SEARCH_PLACEHOLDER}
                            ...
                            <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                                {allStaffLoading ? (
                                    <div className="flex items-center justify-center py-10"><RefreshCw className="w-5 h-5 animate-spin text-blue-400" /></div>
                                ) : filteredStaff.length === 0 ? (
                                    <p className="text-xs text-gray-400 text-center py-8">{UI_STRINGS.STAFF_ENROLL.NO_STAFF_FOUND}</p>
                                ) : filteredStaff.map((record, idx) => {
                                    const role = record.roles?.[0] || "—";
                                    return (
                                        <button key={record.userId} onClick={() => handleSelectStaff(record)}
                                            className={`cursor-pointer w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${selectedStaff?.userId === record.userId ? "border-blue-400 bg-blue-50 shadow-sm" : "border-gray-100 bg-white hover:border-gray-300 hover:bg-gray-50"}`}>
                                            <div className={`w-9 h-9 rounded-full ${AVATAR_COLORS[idx % AVATAR_COLORS.length]} text-white text-xs font-bold flex items-center justify-center shrink-0`}>{getInitials(record.name) || "?"}</div>
                                            ...
                                            {record.enrollmentStatus === "ENROLLED" ? <CheckCircle className="w-4 h-4 text-emerald-500 shrink-0" /> : record.enrollmentStatus === "PARTIAL" ? <span className="text-[10px] font-bold text-amber-500 shrink-0">{record.photosCount}/5</span> : <AlertTriangle className="w-3.5 h-3.5 text-gray-300 shrink-0" />}
                                        </button>
                                    );
                                })}
                            </div>
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden h-full flex flex-col">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-gray-100">
                                <h2 className="text-sm font-bold text-gray-700 flex items-center gap-2"><FileText className="w-4 h-4 text-blue-500" /> {UI_STRINGS.STAFF_ENROLL.STATUS_HEADER}</h2>
                                <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setTablePage(0); }}
                                    ...
                                    <option value="">{UI_STRINGS.COMMON.ALL_STATUS}</option>
                                    <option value="ENROLLED">{UI_STRINGS.STAFF_ENROLL.ENROLLED}</option>
                                    <option value="NOT_ENROLLED">{UI_STRINGS.STAFF_ENROLL.NOT_ENROLLED}</option>
                                    <option value="PARTIAL">{UI_STRINGS.STAFF_ENROLL.PARTIAL}</option>
                                </select>
                            </div>

                            <div className="overflow-x-auto flex-1">
                                {allStaffLoading ? (
                                    <div className="flex items-center justify-center py-16"><RefreshCw className="w-6 h-6 animate-spin text-blue-400" /></div>
                                ) : (
                                    <table className="w-full text-sm">
                                        <thead>
                                            <tr className="bg-gray-50 border-b border-gray-100">
                                                {UI_STRINGS.STAFF_ENROLL.TBL_HEADERS.map((h) => (<th key={h} className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3 whitespace-nowrap">{h}</th>))}
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-gray-50">
                                            {paginatedStaff.length === 0 ? (
                                                <tr><td colSpan={5} className="text-center py-12 text-sm text-gray-400">{UI_STRINGS.STAFF_ENROLL.NO_MEMBERS}</td></tr>
                                                    ...
                                                    <tr key={staff.userId} className={`hover:bg-blue-50/40 transition-colors ${selectedStaff?.userId === staff.userId ? "bg-blue-50/60" : ""}`}>
                                                        <td className="px-4 py-3">
                                                            <div className="flex items-center gap-2.5">
                                                                <div className={`w-8 h-8 rounded-full ${AVATAR_COLORS[idx % AVATAR_COLORS.length]} text-white text-xs font-bold flex items-center justify-center shrink-0`}>{getInitials(staff.name)}</div>
                                                                <div>
                                                        ...
                                                        <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${ROLE_COLORS[role] || "bg-gray-100 text-gray-600"}`}>{role}</span></td>
                                                        ...
                                                        <td className="px-4 py-3"><ActionButton status={staff.enrollmentStatus} onEnroll={() => handleTableEnroll(staff)} onReEnroll={() => handleTableReEnroll(staff)} /></td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
                            ...
                            <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50 flex-wrap gap-2">
                                <span className="text-xs text-gray-500">{UI_STRINGS.STAFF_ENROLL.PAGE} {tablePage + 1} {UI_STRINGS.COMMON.OF} {tableTotalPages}</span>
                                <div className="flex items-center gap-1">
                                    <button onClick={() => setTablePage((p) => Math.max(0, p - 1))} disabled={tablePage === 0} className="cursor-pointer w-7 h-7 rounded-lg flex items-center justify-center border border-gray-200 bg-white hover:bg-gray-100 disabled:opacity-40 transition-colors"><ChevronLeft className="w-4 h-4 text-gray-600" /></button>
                                    {Array.from({ length: Math.min(tableTotalPages, 5) }).map((_, i) => (
                                        <button key={i} onClick={() => setTablePage(i)} className={`cursor-pointer w-7 h-7 rounded-lg text-xs font-semibold border transition-colors ${tablePage === i ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-100"}`}>{i + 1}</button>
                                    ))}
                                    <button onClick={() => setTablePage((p) => Math.min(tableTotalPages - 1, p + 1))} disabled={tablePage >= tableTotalPages - 1} className="cursor-pointer w-7 h-7 rounded-lg flex items-center justify-center border border-gray-200 bg-white hover:bg-gray-100 disabled:opacity-40 transition-colors"><ChevronRight className="w-4 h-4 text-gray-600" /></button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {selectedStaff && (
                    <div ref={uploadPanelRef} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                        <div className="flex items-center gap-1 mb-5 text-xs font-semibold overflow-x-auto pb-1">
                            {UI_STRINGS.STAFF_ENROLL.STEPS.map((step, i) => (
                                <div key={step} className="flex items-center shrink-0">
                                    <div className={`px-3 py-1.5 rounded-full ${activeStep > i + 1 ? "bg-blue-600 text-white" : activeStep === i + 1 ? "bg-blue-100 text-blue-700 border border-blue-300" : "bg-gray-100 text-gray-400"}`}>{i + 1} · {step}</div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
                            <div className="md:col-span-3">
                                <div className="flex items-start gap-3 p-4 rounded-xl bg-blue-50 border border-blue-100 h-full">
                                    <div className="w-12 h-12 rounded-full bg-blue-600 text-white text-sm font-bold flex items-center justify-center shrink-0">{getInitials(selectedStaff.name)}</div>
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-400 mt-0.5">{UI_STRINGS.STAFF_ENROLL.STATUS_HEADER}</p>
                                        <p className="text-xs text-gray-400">{UI_STRINGS.COMMON.ID_LABEL} {selectedStaff.userId}</p>
                                        <div className="mt-2"><StatusBadge status={selectedStaff.enrollmentStatus} photos={selectedStaff.photosCount} /></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-6">
                                <div className="flex items-center justify-between mb-2">
                                    <p className="text-xs font-bold text-gray-700 uppercase tracking-wide">{UI_STRINGS.STAFF_ENROLL.ADD_FACE} <span className="text-red-500">(REQUIRED)</span></p>
                                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${uploadedCount === 5 ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}`}>{uploadedCount}/5</span>
                                </div>
                                <div className="grid grid-cols-5 gap-2">
                                    {Array(5).fill(0).map((_, i) => (<PhotoSlot key={i} index={i} file={photos[i]} onAdd={handleAddPhoto} onRemove={handleRemovePhoto} />))}
                                </div>
                                <div className="mt-3">
                                    <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                        <div className="h-full rounded-full transition-all duration-500 bg-blue-500" style={{ width: `${(uploadedCount / 5) * 100}%` }} />
                                    </div>
                                </div>
                            </div>

                            <div className="md:col-span-3 flex flex-col gap-3">
                                <button onClick={handleClearAll} className="cursor-pointer w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 text-sm font-semibold transition-colors"><Trash2 className="w-4 h-4" /> {UI_STRINGS.STAFF_ENROLL.BTN_CLEAR}</button>
                                <button onClick={handleEnroll} disabled={uploadedCount < 5 || enrolling}
                                    ...
                                    {enrolling ? <><RefreshCw className="w-4 h-4 animate-spin" /> {UI_STRINGS.STAFF_ENROLL.ENROLLING}</> : <><Zap className="w-4 h-4" /> {UI_STRINGS.STAFF_ENROLL.BTN_ENROLL_STAFF}</>}
                                </button>
                            </div>
                        </div>
                    </div>
