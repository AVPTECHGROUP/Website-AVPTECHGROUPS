...
const Leaves = () => {
  ...
  const [resetFiltersCallApiDependency, setresetFiltersCallApiDependency] =
    useState(0);
  ...
  useEffect(() => {
    let fetchListOfValues = async () => {
      try {
        ...
        const formattedStatus = leaveStatusRes
          .filter(
            (item) => item.isActive === true && item.value !== 'WITHDRAWN'
          )
          .map((item) => ({
            id: item.id,
            value: item.value,
  ...
  const fetchWithFilters = async () => {
    ...
    try {
      ...
      if (leavestatusFilter !== 'All Status')
        filters.status = leavestatusFilter;
  ...
  const cardsArray = [
    {
      IconName: ClockIcon,
      keyName: 'Pending request',
      val: statistics.pendingRequests,
      iconTxColor: 'text-orange-600',
      iconBgColor: 'bg-orange-50',
    },
    {
      IconName: ThumbsUpIcon,
      keyName: 'Approved Leaves month',
    ...
    },
    {
      ...
      keyName: 'Total leaves month',
      val: statistics.totalThisMonth,
  ...
  const handleLeaveRejectReq = async () => {
    try {
      const res = await approoveRejLeaveReq(
        selectedUser.leaveId,
        remarkVal,
        'REJECTED'
      );
      toast.success(res.message);
        ...
        <div className="flex-1 bg-linear-to-b from-sky-50 to-sky-100 overflow-auto p-4 pt-2 lg:pt-4 sm:p-6 lg:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1 font-medium text-sm sm:text-base">
                Review and manage employee leave requests for the current
                academic session.
              </p>
            </div>
          ...
          <div className="bg-white grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 px-4 py-3 rounded-xl border border-gray-200 mb-4">
            ...
            <div className="flex gap-2 col-span-2 items-center align-middle">
              <input
                value={frommDateFilter}
                onChange={(e) => setfrommDateFilter(e.target.value)}
                type="date"
                className="px-4 py-2 border border-gray-200 bg-gray-100 rounded-lg focus:outline-none focus:shadow-sm focus:shadow-blue-200 text-sm w-full"
              />
            ...
            <button
              ...
              className="bg-blue-400 rounded-lg text-white sm:text-xs lg:text-sm hover:bg-blue-500 cursor-pointer px-2"
            >
              Apply
            </button>
            <button
              onClick={() => resetFiltersCallApi()}
              className="bg-gray-200 rounded-lg text-black text-sm py-1 hover:bg-gray-300 cursor-pointer px-2"
            >
              Reset
            </button>
          </div>
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-10 h-10 rounded-full ${getAvatarColor(emp.name)} flex items-center justify-center text-white font-semibold shrink-0`}
                              >
                                {emp.avatar}
                              </div>
                ...
                leaveReq.map((emp) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
                      <div
                        className={`w-12 h-12 rounded-full ${getAvatarColor(emp.name)} flex items-center justify-center text-white font-semibold`}
                      >
                        {emp.avatar}
                      </div>
                    ...
                    <p className="text-sm text-gray-700 pb-4">
                      <li className="ml-6">{emp.reason}</li>
                    </p>
                    <button
                      onClick={() => handleViewClick(emp)}
                      className="w-full px-4 py-2 bg-blue-50 text-blue-700 rounded-lg text-sm font-medium hover:bg-blue-100 transition-colors"
                    >
                      View Details
                    </button>
                  </div>
