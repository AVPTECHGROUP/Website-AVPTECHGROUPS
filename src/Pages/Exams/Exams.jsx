...
function ExamCard({ exam, onAction, isSelected, onClick }) {
    ...
    return (
        <div
            onClick={onClick}
            className={`p-4 border-b border-gray-100 last:border-0 transition-colors cursor-pointer ${isSelected ? "bg-blue-50/50 border-l-4 border-l-blue-500" : "hover:bg-blue-50/20"}`}
        >
...
function ErrorToast({ message, onClose }) {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 bg-red-500 text-white text-sm font-medium px-4 py-3 rounded-xl shadow-lg flex items-center gap-2 animate-in slide-in-from-bottom-2 max-w-xs">
            <AlertCircle className="w-4 h-4 shrink-0" />
            <span className="flex-1">{message}</span>
            <button onClick={onClose} className="ml-2 text-white/80 hover:text-white text-lg leading-none shrink-0">×</button>
        </div>
    );
}
...
export default function Exams() {
    ...
    const handleDeclareConfirm = async () => {
        ...
        try {
            ...
            setExams((prev) =>
                prev.map((e) =>
                    e.id === declareTarget.id ? { ...e, resultDeclared: true, ...updatedExam } : e
                )
            );
            setSelectedExamForSubjects((prev) =>
                prev?.id === declareTarget.id ? { ...prev, resultDeclared: true, ...updatedExam } : prev
            );
    ...
    // ─────────────────────────────────────────────────────────────────────────
    return (
        <div className="min-h-screen bg-[#f3f6fb] p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-5">
            ...
            {errorMeta && (
                <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-4 py-3 flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="text-red-600 hover:text-red-800 text-xs font-medium underline whitespace-nowrap"
                    >
                        Refresh
                    </button>
                </div>
            ...
            {/* ── Stats Row: 2×2 on mobile/tablet, 4×1 on lg+ ── */}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-4 sm:px-5 py-4">
                ...
                {/* Bottom row: New Exam button — full width on mobile, right-aligned on sm+ */}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-gray-100 gap-3">
                    <h2 className="text-sm sm:text-base font-semibold text-gray-800 min-w-0">
                ...
                {errorExams && (
                    <div className="px-4 sm:px-6 py-4 text-sm text-red-600 bg-red-50 flex items-center gap-2">
                        <AlertCircle className="w-4 h-4 shrink-0" />
                        <span className="flex-1">{errorExams}</span>
                        <button
                            onClick={fetchExams}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                {errorSubjects && (
                    <div className="px-4 sm:px-6 py-3 text-sm text-red-600 bg-red-50 flex items-center gap-2">
                        <AlertCircle className="w-4 h-4 shrink-0" />
                        <span className="flex-1">{errorSubjects}</span>
                        <button
                        ...
                        <div key={sub.id} className="p-4 flex items-start justify-between gap-3">
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                    ...
                                    <span className="font-semibold text-sm text-gray-800">{sub.subjectName}</span>
                                    <span className="text-xs text-gray-400 font-mono">{sub.subjectCode}</span>
                                </div>
                        ...
                        <thead>
                            <tr className="bg-gray-50 border-y border-gray-100">
                                {["Subject", "Code", "Section", "Max Marks", "Pass Marks", "Theory / Practical", "Actions"].map((h) => (
                                    <th key={h} className="text-left px-4 xl:px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider whitespace-nowrap">
                                        {h}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody>
                            {loadingSubjects ? (
                                ...
                                <tr key={sub.id} className="border-b border-gray-50 hover:bg-blue-50/20 transition-colors">
                                    ...
                                    <td className="px-4 xl:px-6 py-3 text-gray-500 font-mono text-xs">{sub.subjectCode ?? "—"}</td>
                                    <td className="px-4 xl:px-6 py-3 text-gray-600">{sub.sectionName ?? "—"}</td>
                                    <td className="px-4 xl:px-6 py-3 font-bold text-gray-800">{sub.maxMarks}</td>
                                    <td className="px-4 xl:px-6 py-3 text-gray-600">{sub.passingMarks}</td>
                                    <td className="px-4 xl:px-6 py-3">
                                        {sub.hasTheoryPractical ? (
                                            <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700">
                                                Theory {sub.maxTheoryMarks} + Practical {sub.maxPracticalMarks}
