    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-gray-50 flex flex-wrap items-center gap-3">
        <div className="relative">
          {loadingRoutes ? (
            <div className="w-44 h-9 bg-gray-100 animate-pulse rounded-xl" />
          ) : (
            <>
              <select
                ...
                disabled={loading}
                className="appearance-none pl-4 pr-9 py-2 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-200 cursor-pointer min-w-44 disabled:opacity-50 disabled:cursor-not-allowed"
              >
      ...
      <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-3 gap-4 border-b border-gray-200">
        {loading
          ...
          : [
              ...
              <div key={c.label} className="rounded-xl border border-gray-200 p-5 text-center">
              ...
              <tbody className="divide-y divide-gray-50">
                {routeSummaryRows.map((r) => (
                  <tr key={r.routeId} className="hover:bg-gray-50">
                    <td className="py-2.5 pr-4 text-gray-700 text-xs">{r.routeName}</td>
                    <td className="py-2.5 pr-4 font-semibold text-gray-800">{r.studentCount}</td>
                    <td className="py-2.5 pr-4 text-gray-700">{fmt(r.totalMonthlyRevenue)}</td>
                    <td className="py-2.5 text-gray-700">{fmt(r.totalAnnualRevenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                {freqRows.map(([freq, count]) => (
                  <tr key={freq} className="hover:bg-gray-50">
                    <td className="py-2.5 pr-4">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${freqColors[freq] ?? "bg-gray-100 text-gray-600"}`}>
                        {freq}
                      </span>
                    </td>
      ...
      <div className="sm:hidden divide-y divide-gray-100">
        ...
        {/* Mobile rows */}
        {!loading && students.map((s) => (
          <div key={s.allocationId} className="px-4 py-4 space-y-1.5">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <span className="font-bold text-gray-900 truncate">{s.studentName}</span>
                <span className="text-xs text-gray-400 font-normal ml-1">#{s.studentId}</span>
                <p className="text-xs text-gray-400 mt-0.5">{s.className} {s.sectionName}</p>
              </div>
              <span className="bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold px-2 py-0.5 rounded-full shrink-0">
                {s.routeCode}
              </span>
            </div>
            <div className="flex flex-wrap gap-2 text-xs">
              <span className={`font-semibold px-2 py-0.5 rounded-full ${typeColors[s.pickupDropType] ?? "bg-gray-100 text-gray-600"}`}>
                {typeLabel[s.pickupDropType] || s.pickupDropType}
              </span>
              <span className={`font-semibold px-2 py-0.5 rounded-full ${freqColors[s.feeFrequency] ?? "bg-gray-100 text-gray-600"}`}>
                {s.feeFrequency}
              </span>
            </div>
            <div className="flex flex-wrap gap-3 text-xs text-gray-500">
              <span>{s.stopName}</span>
              <span>{s.feePlanName}</span>
              <span className="font-bold text-gray-800">{fmt(s.feeAmount)}</span>
            </div>
          </div>
        ))}
