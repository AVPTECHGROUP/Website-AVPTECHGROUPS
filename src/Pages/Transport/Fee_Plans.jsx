...
import {
  CreditCard, Search, ChevronDown, Plus, Pencil,
  ToggleLeft, ToggleRight, SlidersHorizontal,
} from "lucide-react";
import ActionDropDownComp from "../../Components/CommonComp/ActionDropDownComp";
import AddFeePlanCard from "../../Components/Transport/AddFeePlanCard";

const initialPlans = [
  {
    id: 1, planName: "Route A Monthly Fee", route: "RT-001",
    amount: 1200, frequency: "MONTHLY",
    monthlyEquiv: 1200, annualEquiv: 14400,
    distanceSlab: "—", status: "ACTIVE",
  },
  {
    id: 2, planName: "Route B Monthly Fee", route: "RT-002",
    amount: 900, frequency: "MONTHLY",
    monthlyEquiv: 900, annualEquiv: 10800,
    distanceSlab: "—", status: "ACTIVE",
  },
  {
    id: 3, planName: "Annual Distance Slab – 5km", route: "Generic",
    amount: 8000, frequency: "ANNUALLY",
    monthlyEquiv: 667, annualEquiv: 8000,
    distanceSlab: "5 km", status: "ACTIVE",
  },
];
...
const FREQ_OPTIONS   = ["All Frequencies", "MONTHLY", "QUARTERLY", "ANNUALLY", "ONE-TIME"];
...
function calcEquiv(amount, frequency) {
  switch (frequency) {
    case "MONTHLY":   return { monthly: amount, annual: amount * 12 };
    case "QUARTERLY": return { monthly: Math.round(amount / 3), annual: amount * 4 };
    case "ANNUALLY":  return { monthly: Math.round(amount / 12), annual: amount };
    default:          return { monthly: amount, annual: amount };
  }
}
...
export default function Fee_Plans() {
  ...
  const handleSave = (data) => {
    const routeCode = data.route
      ? data.route.split(" – ")[0]
      : "Generic";
    const equiv = calcEquiv(data.feeAmount, data.frequency);
    const newPlan = {
      id: plans.length + 1,
      ...
      monthlyEquiv: equiv.monthly,
      annualEquiv: equiv.annual,
      distanceSlab: data.distanceSlab ? `${data.distanceSlab} km` : "—",
      status: "ACTIVE",
    };
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {["Plan Name", "Route", "Amount", "Frequency", "Monthly Equiv.", "Annual Equiv.", "Distance Slab", "Status", "Actions"].map((h) => (
                ...
                {paginated.length === 0 ? (
                  ...
                  paginated.map((p) => (
                    <tr key={p.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      {/* Monthly Equiv */}
                      <td className="px-4 py-4 text-gray-700 text-center font-medium">{fmt(p.monthlyEquiv)}</td>
                      {/* Annual Equiv */}
                      <td className="px-4 py-4 text-gray-700 font-medium text-center">{fmt(p.annualEquiv)}</td>
                      {/* Distance Slab */}
                      ...
                      <td className="px-4 py-4">
                        <ActionDropDownComp
                          onAction={(val) => handleAction(p.id, val)}
                          actionOptions={[
              ...
              paginated.map((p) => (
                <div key={p.id} className="px-4 py-4 space-y-3 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex gap-4 text-xs text-gray-500">
                    <span>Monthly: <span className="font-semibold text-gray-700">{fmt(p.monthlyEquiv)}</span></span>
                    <span>Annual: <span className="font-semibold text-gray-700">{fmt(p.annualEquiv)}</span></span>
                  </div>
                  <div className="pt-1">
                    <ActionDropDownComp
                      onAction={(val) => handleAction(p.id, val)}
