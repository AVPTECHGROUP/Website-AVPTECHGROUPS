...
export default function Fee_Plans() {
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f8] font-sans w-full max-w-full overflow-x-hidden min-w-0 flex flex-col">
      ...
      {/* Page Header */}
      <div className="px-4 sm:px-6 xl:px-8 pt-8 pb-2 w-full max-w-full">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden w-full max-w-full min-w-0 flex flex-col">
          ...
          <div className="px-4 sm:px-6 py-3.5 border-b border-gray-50 flex flex-col md:flex-row gap-3 w-full max-w-full min-w-0 items-center">
            <div className="relative flex-1 w-full">
              ...
              <input
                type="text"
                placeholder="Search plan name or route..."
                value={search}
          ...
          {/* Desktop & Laptop Table - Hidden under 1280px (xl breakpoint) */}
          ...
          {/* Mobile & Tablet Adaptive Cards - Visible under 1280px (xl breakpoint) */}
                ...
                <div key={p.id} className="p-4 md:p-5 space-y-3 bg-white hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5 font-medium">{p.route === "Generic" ? "Generic / Distance Plan" : `Route: ${p.route}`}</p>
                    </div>
                    {p.status === "ACTIVE" ? (
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 bg-gray-50 p-2.5 rounded-xl text-xs">
                    <div className="flex items-center gap-2">
                      <span className={`font-bold px-2.5 py-0.5 rounded-full text-[11px] ${freqColors[p.frequency] ?? "bg-gray-100 text-gray-600"}`}>{p.frequency}</span>
                      {p.distanceSlab !== "—" && <span className="text-gray-500 font-medium">Slab: {p.distanceSlab}</span>}
                    </div>
