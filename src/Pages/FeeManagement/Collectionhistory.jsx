...
const CollectFeeModal = ({ open, onClose, student, onSuccess }) => {
  ...
  useEffect(() => {
    if (open && student) {
      setForm({
        ...
        paymentMode: 'CASH', paymentDate: todayISO(),
        referenceNo: '', discount: '', discountReason: '', lateFine: '', remarks: '',
      });
    }
  }, [open, student]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await createFeeCollection(payload);
      onSuccess(response, student);
    } catch (e) {
      alert(e.message || 'Failed to record payment. Please try again.');
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#EEF4FF] border border-[#C7D7EE] rounded-xl p-3 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-gray-500 mt-0.5">
                Parent: {student.parentName || 'N/A'} · {student.parentPhone || '—'}
              </div>
            </div>
          </div>
          {/* Period */}
          <div>
            ...
            <select className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50 text-gray-600" disabled>
              <option>{student.period} — Balance: {fmt(student.balance)}{isOverdue ? ' (Overdue)' : ''}</option>
            </select>
          </div>
          {/* Breakdown */}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button type="button"
                onClick={() => setForm((p) => ({ ...p, amountPaid: student.balance?.toString() }))}
          ...
          <div>
            ...
            <Inp type="number" value={form.discount} placeholder="Discount amount e.g. 500"
              ...
              <div className="flex items-start gap-2 mb-2">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-amber-700 mt-0.5">
                    Due was {fmtDate(student.dueDate)} · {student.daysLate} day{student.daysLate !== 1 ? 's' : ''} overdue
                  </div>
                </div>
              </div>
              <label className="block text-[11.5px] font-semibold text-amber-800 mb-1">Late Fine Amount (₹)</label>
              <Inp type="number" value={form.lateFine} placeholder="Enter manually e.g. 100 — leave blank to waive"
...
const BulkCollectModal = ({ open, onClose, students, onSuccess }) => {
  ...
  useEffect(() => {
    if (open && students.length > 0) {
      ...
      setRows(students.map((s) => ({
        ...
        daysLate: s.daysLate || 0, paymentMode: 'CASH',
        paymentDate: todayISO(), referenceNo: '', remarks: '',
      })));
    }
  }, [open, students]);
  ...
  const applyToAll = () =>
    setRows((prev) => prev.map((r) => ({ ...r, paymentMode: commonMode, paymentDate: commonDate })));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await createBulkFeeCollection(payload);
      onSuccess(response);
    } catch (e) {
      alert(e.message || 'Failed to process bulk payments.');
    } finally { setLoading(false); }
  };

  const modeOpts = [
    { value: 'CASH', label: 'Cash' }, { value: 'ONLINE', label: 'Online' },
    { value: 'CHEQUE', label: 'Cheque' }, { value: 'DD', label: 'DD' },
  ];
    ...
    <Modal open={open} onClose={onClose} title={`Bulk Fee Collection — ${students.length} Students`} wide
      footer={
        <>
          <Btn variant="secondary" onClick={onClose}>Cancel</Btn>
          <Btn variant="success" onClick={handleSubmit} disabled={loading}>
            {loading ? 'Processing…' : `Process ${rows.length} Payments & Generate Receipts`}
          </Btn>
        </>
      }>
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-2.5 mb-4 text-sm text-emerald-800">
        Payments will be recorded for all {students.length} selected students. Adjust amounts, discounts and late fines per row as needed.
      </div>
      {/* Common controls */}
      <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 mb-4 flex items-end gap-4">
        <div>
          ...
          <Sel value={commonMode} onChange={setCommonMode} options={modeOpts} className="w-32" />
        </div>
        <div>
          <label className="block text-[11.5px] font-semibold text-gray-600 mb-1">Payment Date</label>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className={`border-b border-gray-100 ${row.daysLate > 0 ? 'bg-red-50' : ''}`}>
                ...
                <td className="px-3 py-2.5">
                  <span className="px-2 py-0.5 bg-[#EEF4FF] text-[#1A3A5C] text-xs font-semibold rounded">{row.class}</span>
                </td>
...
const ReceiptModal = ({ open, onClose, receipt }) => {
  ...
  const handlePrint = () => {
    ...
    win.document.write(`<html><head><title>Receipt ${receipt?.receiptNo || ''}</title>
      ...
      .paid{color:#0D7A55;border:2px solid #0D7A55;display:inline-block;
            padding:3px 18px;font-size:16px;font-weight:900;letter-spacing:.15em;
            transform:rotate(-6deg);margin-top:14px;font-family:sans-serif}
    ...
    <Modal open={open} onClose={onClose} title="Payment Receipt"
      footer={
        <>
          ...
          <Btn variant="ghost"><FileText size={13} /> PDF</Btn>
        </>
      }>
      {receipt && (
        <div ref={printRef} className="border-2 border-gray-300 rounded-xl p-6 font-mono text-xs max-w-sm mx-auto bg-gray-50">
          ...
          <div className="flex justify-between font-bold my-2">
            <span>FEE RECEIPT</span><span>{receipt.receiptNo}</span>
          </div>
          <hr className="border-dashed border-gray-300" />
          <div className="space-y-1 my-2">
            <div className="flex justify-between"><span>Date:</span><span>{fmtDate(receipt.date)}</span></div>
            <div className="flex justify-between"><span>Student:</span><span>{receipt.studentName}</span></div>
            <div className="flex justify-between"><span>Class:</span><span>{receipt.class || receipt.className}</span></div>
            <div className="flex justify-between"><span>Adm. No.:</span><span>{receipt.studentCode}</span></div>
            <div className="flex justify-between"><span>Period:</span><span>{receipt.period || receipt.periodName}</span></div>
          </div>
          <hr className="border-dashed border-gray-300" />
          <div className="space-y-1 my-2">
            {(receipt.components || []).map((c, i) => (
              <div key={i} className="flex justify-between">
                <span>{c.name || c.componentType}</span><span>{fmt(c.amount)}</span>
              </div>
            ))}
            {receipt.lateFine > 0 && (
              <div className="flex justify-between text-amber-700 font-semibold">
                <span>Late Fine</span><span>{fmt(receipt.lateFine)}</span>
              </div>
            )}
            {receipt.discount > 0 && (
          ...
          <div className="space-y-1 my-2">
            <div className="flex justify-between font-bold">
              <span>TOTAL COLLECTED</span><span>{fmt(receipt.amountPaid)}</span>
            </div>
            ...
            <div className="flex justify-between text-emerald-700 font-semibold">
              <span>Balance After:</span><span>{fmt(receipt.balanceAfter)}</span>
            </div>
          </div>
          <hr className="border-dashed border-gray-300" />
          <div className="flex justify-between text-[10px] text-gray-400 font-sans mt-2">
            <span>By: {receipt.recordedBy || 'Admin'}</span>
            <span>{fmtDate(receipt.date)}</span>
          </div>
          <div className="text-center mt-4">
            <span className="inline-block text-emerald-700 border-2 border-emerald-700 px-5 py-1 font-black text-base tracking-widest -rotate-[6deg] font-sans">
              PAID
            </span>
          </div>
        </div>
...
const [totalPages, setTotalPages] = useState(1);
  ...
  useEffect(() => {
    const loadClasses = async () => {
      try {
        const res = await authFetch(`${BASE_URL}/classes/school/${SCHOOL_ID}/active`);
        if (!res.ok) return;
        const data = await res.json();
        const records = Array.isArray(data) ? data : (data.data || []);
        setClassOptions(records.map((c) => ({ value: String(c.id), label: c.name || c.className })));
      } catch (e) { console.error('Classes load failed:', e); }
    };
    const loadPeriods = async () => {
      try {
        const periods = await getFeePeriods(ACADEMIC_YEAR_ID);
        setPeriodOptions(periods.map((p) => ({ value: String(p.id), label: p.name || p.periodName })));
      } catch (e) { console.error('Periods load failed:', e); }
    };
    loadClasses();
    loadPeriods();
  }, []);
...
const fetchOutstanding = async () => {
  try {
    ...
    setOutstanding(
      (res.records || []).map((r, index) => {
        ...
        if ((r.paidAmount || 0) > 0 && r.balanceDue > 0) {
          status = 'PARTIAL';
        }
  ...
  if (tab === "outstanding") {
    ...
    // only call after dates are ready
    if (fromDate && toDate) {
      fetchHistory();
    }
  }
}, [tab, fromDate, toDate, classF, periodF, modeF, page]);
    ...
    try {
      ...
      setReceiptModal({
        ...
        receipt: {
          ...
          referenceNo: item.referenceNo, balanceAfter: 0,
  ...
  const resetTab = () => {
    ...
    setStatusF(''); setModeF(''); setFromDate(''); setToDate('');
    setPage(1); setSelected([]); setError(null);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Collect fee payments and view complete transaction history</p>
        </div>
        <div className="flex gap-2">
          <Btn variant="secondary" size="sm"><FileDown size={13} /> Export</Btn>
          {/* Opens first outstanding student; if none yet loaded shows disabled */}
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 flex items-start gap-3">
          ...
          <button onClick={() => { setError(null); tab === 'outstanding' ? fetchOutstanding() : fetchHistory(); }}
            className="text-red-600 hover:text-red-800 font-semibold text-sm">Retry</button>
        </div>
      )}
        ...
        {[
          ...
          <button key={t.key} onClick={() => { setTab(t.key); resetTab(); }}
            className={`inline-flex items-center gap-2 px-4 py-3 text-[12.5px] font-semibold border-b-2 -mb-px transition-colors ${
              tab === t.key ? 'text-[#1A3A5C] border-[#1A3A5C]' : 'text-gray-500 border-transparent hover:text-gray-700'
            }`}>
            {t.label}
            {t.badge > 0 && (
              <span className="bg-red-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{t.badge}</span>
            )}
          </button>
        ))}
      </div>
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={10} className="text-center py-12">
                      <div className="animate-spin w-7 h-7 border-2 border-[#1A3A5C] border-t-transparent rounded-full mx-auto mb-2" />
                    ...
                    return (
                      <tr key={s.id} className={`border-b border-gray-100 transition-colors ${
                        isSel ? 'bg-blue-50 border-l-4 border-l-[#1A3A5C]' :
                        s.status === 'OVERDUE' ? 'bg-red-50/40 hover:bg-red-50' : 'hover:bg-gray-50'
                      }`}>
                        ...
                        <td className="px-3 py-3">
                          <span className="px-2 py-0.5 bg-[#EEF4FF] text-[#1A3A5C] text-xs font-semibold rounded">{s.class}</span>
                        </td>
                        ...
                        <td className="px-3 py-3 text-xs">
                          <span className={s.status === 'OVERDUE' ? 'text-red-600 font-semibold' : 'text-gray-600'}>
                            {fmtDate(s.dueDate)}
                          </span>
                        </td>
                        <td className="px-3 py-3"><Badge status={s.status} /></td>
                        <td className="px-3 py-3">
                          <Btn variant={s.status === 'OVERDUE' ? 'danger' : 'primary'} size="xs"
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={h.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-3 py-3">
                        <span className="px-2 py-0.5 bg-[#EEF4FF] text-[#1A3A5C] text-xs font-semibold rounded">{h.class}</span>
                      </td>
                      ...
                      <td className="px-3 py-3">
                        <Btn variant="ghost" size="xs" onClick={() => handleViewReceipt(h)}>Receipt</Btn>
                      </td>
                    </tr>
                  ))}
                </tbody>
