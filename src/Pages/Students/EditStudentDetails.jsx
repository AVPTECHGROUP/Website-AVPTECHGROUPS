...
function EditStudentDetails() {
  ...
  const handleImageChange = (e) => {
    ...
    if (!validTypes.includes(file.type)) { toast.error(ES.ERRORS.PHOTO_TYPE); return; }
    if (file.size > 10 * 1024 * 1024) { toast.error(ES.ERRORS.PHOTO_SIZE); return; }
  ...
  useEffect(() => {
    const fetchSections = async () => {
      try {
        ...
        toast.error(ES.ERRORS.SECTION_LOAD_RETRY);
      } finally {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        ...
        toast.error(ES.ERRORS.LOAD_FAILED);
      } finally {
        setLoading(false);
      }
    };
  ...
  const validateForm = () => {
    ...
    if (!formData.name || !formData.mobile || !formData.gender) { toast.error(ES.ERRORS.REQUIRED_FIELDS); return false; }
    if (!formData.rollNumber?.trim()) { toast.error(ES.ERRORS.ROLL_REQUIRED); return false; }
    if (!formData.sectionId) { toast.error(ES.ERRORS.SECTION_REQUIRED); return false; }
    if (!mobileRegex.test(formData.mobile)) { toast.error(ES.ERRORS.MOBILE_INVALID); return false; }
    if (formData.emergencyContact && !mobileRegex.test(formData.emergencyContact)) { toast.error(ES.ERRORS.EMERGENCY_INVALID); return false; }
    if (formData.fatherPhone && !mobileRegex.test(formData.fatherPhone)) { toast.error(ES.ERRORS.FATHER_PHONE_INVALID); return false; }
    if (formData.email) {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(formData.email)) { toast.error(ES.ERRORS.EMAIL_INVALID); return false; }
    }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const loadingToast = toast.loading(ES.LOADING);
    try {
      const payload = buildUpdatePayload();
      const res = await updateStudent(id, payload, profileImage);
      toast.dismiss(loadingToast);
      toast.success(res.message || ES.SUCCESS);
      if (profileImage) toast.info(ES.PROFILE_PHOTO.REFRESH_NOTICE, { autoClose: 4000 });
      navigate('/students');
    } catch (error) {
      toast.dismiss(loadingToast);
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex justify-start items-center mb-4 pb-3 border-b border-gray-200">
                    ...
                    <h2 className="text-xl font-medium text-gray-700">{ES.PROFILE_PHOTO.TITLE}</h2>
                  </div>
                    ...
                    <div className="flex-1">
                      {!displayedImage ? (
                        <button type="button" onClick={() => fileInputRef.current?.click()}
                          ...
                          <p className="text-sm font-medium text-blue-600">{ES.PROFILE_PHOTO.CTA}</p>
                          <p className="text-xs text-gray-400 mt-0.5">{ES.PROFILE_PHOTO.FORMAT_HELP}</p>
                        </button>
                      ) : (
                            ...
                            {profileImage ? (
                              <>
                                ...
                                <p className="text-xs text-green-500 mt-0.5">
                                  {(profileImage.size / 1024).toFixed(1)} KB — {ES.PROFILE_PHOTO.NEW_SELECTED}
                                </p>
                              </>
                            ) : (
                              <>
                                <p className="text-sm font-medium text-green-700 truncate">{ES.PROFILE_PHOTO.CURRENT}</p>
                                <p className="text-xs text-green-500 mt-0.5">{ES.PROFILE_PHOTO.REPLACE}</p>
                              </>
                            )}
                          </div>
                ...
                <div>
                  ...
                  <div className="grid lg:grid-cols-2 sm:grid-cols-1 gap-4">
                    <div>
                      <label className={labelClass}>{PF.FULL_NAME}<span className="text-red-600 ml-1">*</span></label>
                      <input type="text" name="name" value={formData.name} onChange={handleInputChange}
                        placeholder="Enter full name" className={inputClass} required />
                    </div>
                    <div>
                    ...
                    <div className="lg:col-span-2">
                      ...
                      <textarea rows={4} name="address" value={formData.address} onChange={handleInputChange}
                ...
                <div>
                  ...
                  <div className="grid lg:grid-cols-2 sm:grid-cols-1 gap-6">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {formData.hostelRequired ? PF.HOSTEL_ENABLED : PF.HOSTEL_DISABLED}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {formData.transportRequired ? PF.TRANSPORT_ENABLED : PF.TRANSPORT_DISABLED}
                      </p>
                    </div>
                  </div>
                </div>
