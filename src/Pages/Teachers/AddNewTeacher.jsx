...
function AddNewTeacher() {
    ...
    const validatePersonalDetails = () => {
        const newErrors = {};
        if (!formData.name.trim()) newErrors.name = strings.ADD_TEACHER.VALIDATION.FULL_NAME_REQUIRED;
        else if (formData.name.trim().length < 2) newErrors.name = strings.ADD_TEACHER.VALIDATION.NAME_MIN_LENGTH;
        if (!formData.gender) newErrors.gender = strings.ADD_TEACHER.VALIDATION.GENDER_REQUIRED;
        if (!formData.mobile) newErrors.mobile = strings.ADD_TEACHER.VALIDATION.MOBILE_REQUIRED;
    ...
    const validateSalaryDetails = () => {
        const newErrors = {};
        if (!formData.salaryType) newErrors.salaryType = strings.ADD_TEACHER.VALIDATION.SALARY_TYPE_REQUIRED;
        if (!formData.baseSalary || Number(formData.baseSalary) <= 0) newErrors.baseSalary = strings.ADD_TEACHER.VALIDATION.BASE_SALARY_INVALID;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const apiPayload = {
                personalDetails: {
                    fullName: formData.name, mobile: formData.mobile, email: formData.email,
            ...
            if (isPayrollEnabled && response && formData.salaryType && formData.baseSalary) {
                ...
                if (teacherId) {
                    ...
                    const salaryPayload = {
                        ...
                        effectiveFrom: today, effectiveTo, payrollEligible: true, remarks: "Created via AddNewTeacher",
                        grossSalary, totalDeductions, netSalary
                    };
                    try { await upsertTeacherSalary(teacherId, salaryPayload); } catch (salaryError) { console.error(salaryError); }
                }
            }
            toast.dismiss(loadingToast);
            toast.success(strings.ADD_TEACHER.SUCCESS);
            if (profileImage) toast.info(strings.EDIT_TEACHER.PHOTO_REFRESH_NOTICE, { autoClose: 4000 });
        ...
        <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-4">
            <div className="mx-auto">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm sm:text-base text-gray-500">{strings.ADD_TEACHER.SUBTITLE}</p>
                </div>
                            ...
                            <nav className="flex flex-wrap -mb-px">
                                ...
                                {isPayrollEnabled && (
                                    <button type="button" onClick={() => {
                                        const isValid = validatePersonalDetails();
                                        if (!isValid) { toast.error(strings.ADD_TEACHER.COMPLETE_PERSONAL); return; }
                                        setActiveTab('salary');
                                ...
                                <>
                                    <div className="mb-6">
                                        <label className="block font-semibold text-gray-600 text-sm mb-3">
                                            {strings.ADD_TEACHER.UPLOAD.LABEL} <span className="text-gray-400 text-xs font-normal ml-1">{strings.ADD_TEACHER.UPLOAD.HELP}</span>
                                        </label>
