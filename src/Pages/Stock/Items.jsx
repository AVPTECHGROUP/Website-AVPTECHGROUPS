...
export default function Items() {
    ...
    const [pagination, setPagination] = useState({
        currentPage: 0, totalPages: 1, totalElements: 0,
    ...
    // ── Fetch items (server-side filter + page) ────────────────
    ...
    const stats = useMemo(() => [
        {
            key: "Total Items",
            val: statsLoading ? "…" : (statsData?.totalItems    ?? 0),
            icon: Package,      txColor: "text-blue-600",   bgColor: "bg-blue-50",
        },
        {
            key: "Active Items",
            val: statsLoading ? "…" : (statsData?.activeItems   ?? 0),
            icon: PackageCheck, txColor: "text-green-600",  bgColor: "bg-green-50",
        },
        {
            key: "Inactive",
            val: statsLoading ? "…" : (statsData?.inactiveItems ?? 0),
            icon: PackageX,     txColor: "text-red-500",    bgColor: "bg-red-50",
        },
        {
            key: "Categories",
            val: statsLoading ? "…" : (statsData?.categories    ?? statsData?.totalCategories ?? 0),
            icon: Layers,       txColor: "text-purple-600", bgColor: "bg-purple-50",
        },
    ...
    // Windowed page numbers (0-based internally, display as +1)
    ...
    const clearFilters = () => {
        setSearch(""); setCategoryFilter("All Categories"); setStatusFilter("All Status"); resetPage();
    };
        ...
        <div className="min-h-screen bg-blue-50 p-3 md:p-5 xl:p-8 font-sans">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="flex items-center justify-between gap-3 px-4 md:px-5 py-3 md:py-4 border-b border-gray-100">
                    ...
                    >
                        <Plus className="w-3.5 h-3.5 md:w-4 md:h-4" /> New Item
                    </button>
                </div>
                ...
                {/* ── Filters ──────────────────────────────────────────────
                    ...
                    md–xl : search on top, two selects below
                    < md  : search + toggle button; selects in collapsible drawer
                ── */}
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">
                                ...
                                <th className="px-3 lg:px-4 py-3 text-center">Category</th>
                                <th className="px-3 lg:px-4 py-3 text-center">Unit</th>
                                <th className="px-3 lg:px-4 py-3 text-center">Stock</th>
                                <th className="px-3 lg:px-4 py-3 text-center whitespace-nowrap">Min Lvl</th>
                            ...
                            {loading ? (
                                ...
                                items.map((item, idx) => (
                                    <tr key={item.id} className="hover:bg-blue-50/40 transition-colors">
                                        ...
                                        <td className="px-3 lg:px-4 py-3 text-center text-sm text-gray-600 whitespace-nowrap">
                                            {item.unit}
                                        </td>
                                        <td className="px-3 lg:px-4 py-3 text-center">
                                        ...
                                        <td className="px-3 lg:px-4 py-3 text-center text-sm text-gray-600">
                                            {item.minLevel}
                                        </td>
                                        <td className="px-3 lg:px-4 text-center py-3">
