...
export default function CreateStudentOrder() {
    ...
    // orderItems: { itemId, itemName, itemCode, itemUnit, category, quantity,
    ...
    // Tracks created order id (for confirm after create)
    ...
    // ─── Fetch preview from API when entering step 2 ────────────
    // Preview returns unitPriceSnapshot + lineTotal per item
    ...
    // ─── Refresh only availability (keep prices/items) ──────────
    ...
    // On step 2 entry: if no items yet → fetch preview; else refresh availability
    useEffect(() => {
        if (step !== 2) return;
        ...
        <div className="min-h-screen bg-linear-to-b from-sky-50 to-sky-100">
            <div className="p-3 sm:p-4 lg:p-5 max-w-7xl mx-auto">
                ...
                <div className="flex items-center justify-between gap-4 mb-6">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">New Student Order</h1>
                        <p className="text-gray-500 text-sm mt-0.5">Create a new stock order for a student.</p>
                    </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {selectedStudent?._className && (
                                <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-sm flex-wrap">
                                    ...
                                    <span className="text-xs text-gray-400 hidden sm:block ml-auto shrink-0">· Default items will be pre-loaded from class config</span>
                                </div>
                            )}
                        </div>
