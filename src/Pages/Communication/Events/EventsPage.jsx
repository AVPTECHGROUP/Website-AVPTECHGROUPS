...
export default function EventsPage() {
  ...
  // FIX: toast state — this page had no toast UI at all, so delete/other
  // confirmations were silently lost.
  const [toast, setToast] = useState(null);
  const showToast = (type, msg) => {
    setToast({ type, msg });
    setTimeout(() => setToast(null), COMMUNICATION_CONSTS.TOAST_DURATION_MS ?? 3000);
  };
  ...
  // FIX: this reuses the same cancelEvent API as "reject" (there's no
  // separate delete endpoint), so it now always shows a toast on success —
  // "Deleted successfully" — regardless of whatever EventDetailModal does
  // internally with the returned { error } value.
